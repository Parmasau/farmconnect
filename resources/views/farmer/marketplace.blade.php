...
@section('title', 'Buy from Farmers - FarmNest')
...
<div class="bg-white/95 backdrop-blur-sm rounded-xl shadow p-6">
    <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Products from Other Farmers</h1>
            <p class="text-gray-500 text-sm mt-1">Browse and purchase fresh farm products directly from farmers</p>
        </div>
    ...
    <div class="bg-gray-50 rounded-xl p-4 mb-6">
        <form method="GET" action="{{ route('farmer.products.marketplace') }}" class="flex flex-wrap gap-3">
            ...
            <div class="flex gap-2">
                ...
                @if(request('search') || request('category'))
                    <a href="{{ route('farmer.products.marketplace') }}" class="bg-gray-500 text-white px-6 py-2 rounded-lg hover:bg-gray-600 transition">
                ...
                <div class="p-4">
                    ...
                    <div class="bg-green-50 rounded-lg p-3 mb-3">
                        <div class="flex items-center gap-2">
                            ...
                            <div class="flex-1">
                                <p class="font-semibold text-sm text-gray-800">{{ $product->farmer->name ?? 'Farmer' }}</p>
                                <p class="text-xs text-gray-500">Seller since {{ $product->farmer->created_at ? $product->farmer->created_at->format('M Y') : 'N/A' }}</p>
                            </div>
                            <a href="{{ route('farmer.messages.create', ['farmer_id' => $product->farmer_id, 'product_id' => $product->id]) }}" 
                               class="text-green-600 hover:text-green-700" title="Message Seller">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('farmer.products.view', $product->id) }}" 
                           class="flex-1 bg-green-600 text-white text-center py-2 rounded-lg hover:bg-green-700 transition flex items-center justify-center gap-2">
    ...
    @else
        <div class="text-center py-16">
            ...
            <p class="text-gray-500 mb-6">No products from other farmers at the moment.</p>
            <a href="{{ route('farmer.products.create') }}" class="inline-block bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 transition">
