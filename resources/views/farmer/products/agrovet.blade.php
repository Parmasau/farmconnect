...
<div class="bg-white/95 backdrop-blur-sm rounded-xl shadow p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex gap-3">
            <form method="GET" action="{{ route('farmer.products.agrovet') }}" class="flex gap-2">
                <input type="text" name="search" placeholder="Search products..." 
                       value="{{ request('search') }}"
                       class="border rounded-lg px-3 py-1 text-sm">
                <select name="category" class="border rounded-lg px-3 py-1 text-sm">
                ...
                @if(request('search') || request('category'))
                    <a href="{{ route('farmer.products.agrovet') }}" class="bg-gray-500 text-white px-3 py-1 rounded-lg text-sm">Clear</a>
                @endif
            </form>
        </div>
    </div>
                ...
                <div class="p-4">
                    ...
                    <div class="bg-gray-50 rounded-lg p-3 mb-3">
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                <p class="font-medium text-sm">{{ $product->seller->name ?? 'Agrovet' }}</p>
                                <p class="text-xs text-gray-500">Agrovet</p>
                            </div>
                        </div>
                    </div>
    ...
    @else
        <div class="text-center py-12">
            ...
            <h3 class="text-lg font-medium mb-2">No Agrovet Products Available</h3>
            <p class="text-gray-500 mb-4">Check back later for products from agrovets</p>
        </div>
    @endif
</div>
@endsection